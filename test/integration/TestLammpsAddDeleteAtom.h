...
template <unsigned int dim> class TestLammpsAddDeleteAtom : public TestLammps<dim> {
public:
  TestLammpsAddDeleteAtom(int argc, char** argv, std::string name) : TestLammps<dim>(argc, argv, name) {}
  ...
  virtual void run() {
    ...
    if (performDeletion) {
      ...
      // I01 globalIndex{tarch::la::Vector<3, int>{deleteCellIndex}};
      // I11 localIndex{globalIndex};
      ...
      // delete molecule
      coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell, dim>::getInstance().getMDSolverInterface()->deleteMoleculeFromMDSimulation(
          deleteMolecule, deleteCell);
    ...
    if (performAdd) {
      ...
      // I01 globalIndex{tarch::la::Vector<3, int>{addCellIndex}};
      // I11 localIndex{globalIndex};
...
// 
      ...
      // add molecule
      coupling::interface::MamicoInterfaceProvider<LAMMPS_NS::MamicoCell, dim>::getInstance().getMDSolverInterface()->addMoleculeToMDSimulation(addMolecule);
      ...
      for (int i = 0; i < TestLammps<dim>::_lammps->atom->nlocal; i++) {
        bool found = true;
        for (unsigned int d = 0; d < dim; d++) {
          found = found && tarch::la::equals(TestLammps<dim>::_lammps->atom->x[i][d], posAddAtom[d], tolerance);
          found = found && tarch::la::equals(TestLammps<dim>::_lammps->atom->v[i][d], velAddAtom[d], tolerance);
          found = found && tarch::la::equals(TestLammps<dim>::_lammps->atom->f[i][d], fAddAtom[d], tolerance);
        }
        if (found) {
          foundMolecule = true;
          break;
    ...
    // test synchronisation of mass across processes
    {
      ...
      if (dim == 2) {
        if (totalNumberAtoms != 4) {
          std::cout << "ERROR TestLammpsAddDeleteAtom: total number of atoms=" << totalNumberAtoms << " instead of 4" << std::endl;
          exit(EXIT_FAILURE);
        }
      }
      if (dim == 3) {
        if (totalNumberAtoms != 8) {
          std::cout << "ERROR TestLammpsAddDeleteAtom: total number of atoms=" << totalNumberAtoms << " instead of 8" << std::endl;
          exit(EXIT_FAILURE);
        }
      }
