...
public:
  ...
  virtual void run() {
    ...
    unsigned int* globalCellIndices = initRecvBuffer(recvBuffer, globalNumberCouplingCells, interface, density, dx);
    if (rank == 0) {
    ...
    solver.setMDBoundaryValues(recvBuffer);
    ...
    for (coords[2] = 0; coords[2] < globalNumberCouplingCells[2] + 2; coords[2]++) {
      for (coords[1] = 0; coords[1] < globalNumberCouplingCells[1] + 2; coords[1]++) {
        for (coords[0] = 0; coords[0] < globalNumberCouplingCells[0] + 2; coords[0]++) {
          ...
          if (contained) {
            myIndex.push_back(coupling::indexing::convertToScalar(
              I01{tarch::la::Vector<3, int>{coords}}));
