...
void tarch::utils::RandomNumberService::init(bool fixSeed) {
  ...
  if (fixSeed) {
    simple_rand.seed(10000);
  } else {
    int rank = 0;
#if (TARCH_PARALLEL == TARCH_YES)
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
#endif
    simple_rand.seed(static_cast<unsigned int>(time(NULL)) * (rank + 1));
  }
...
double tarch::utils::RandomNumberService::getUniformRandomNumber() const {
  ...
  return (0.5 * (double)simple_rand()) / (0.5 * (double)RAMAX + 1.0);
}
...
double tarch::utils::RandomNumberService::getGaussianRandomNumber() {
  ...
  if (_isFirstRandomNumber) {
    ...
    while (s >= 1.0) {
      _randomNumbers[0] = ((double)simple_rand()) / ((double)RAMAX);
      _randomNumbers[1] = ((double)simple_rand()) / ((double)RAMAX);
