...
private:
  ...
  tarch::la::Vector<dim, double> getLocalOffset() const {
    ...
    const tarch::la::Vector<dim, unsigned int> thisProcess = coupling::indexing::IndexingService<dim>::getInstance().getThisProcess();
    const tarch::la::Vector<dim, unsigned int> avgNumberCells = coupling::indexing::IndexingService<dim>::getInstance()
